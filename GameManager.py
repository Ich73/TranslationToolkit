...
def rebuildGame(game_dir, game_file, version, dstool, makerom):
	...
	try:
		...
		exefs_dir = join(game_dir, 'ExtractedExeFS')
		if isdir(exefs_dir) and isfile(join(game_dir, headerExe)):
		...
		if all([headerN0, exHeader, exeFS, romFS]):
			...
			if isfile(join(game_dir, logoLZ)):   arguments.append('--logo %s' % logoLZ)
			if isfile(join(game_dir, plainRGN)): arguments.append('--plain %s' % plainRGN)
